...
class HelperProductionDelivery
{
    public static function cetakPdfDlv($data)
    {
        ...
        if ($dataPreference != null) {
            $alamatTagih = $dataPreference->alamat_pt.', '.$dataPreference->kelurahan.', '.$dataPreference->kecamatan.', '.$dataPreference->kota.' - '.$dataPreference->kode_pos;
            $picTagih = $dataPreference->pic_alamat;
            $telpPicTagih = $dataPreference->telp_pic;
            if ($dataPreference->nama_outlet == ""){
                $outletTagih = "";
            }
            else {
                $outletTagih = " - ".$dataPreference->nama_outlet;
            }

            if ($dataPreference->pic_alamat == ""){
                $picTagih = "-";
            }
            else {
                $picTagih = $dataPreference->pic_alamat;
        ...
        $fpdf->MultiCell(65,4,$dataDelivery->nama_supplier,0,'L' );
        $fpdf->ln(1);
        $fpdf->SetFont('Arial','',9);
        ...
        $fpdf->SetXY(75,36);
        $fpdf->SetFont('Arial','B',9);
        $fpdf->MultiCell(65,4,strtoupper($dataPreference->nama_pt),0, 'L' );
        $fpdf->ln(1);
        $fpdf->SetFont('Arial','',9);
        $fpdf->SetX(75);
        ...
        $fpdf->SetXY(140,11);
        $fpdf->Cell(65,7,"SURAT JALAN PRODUKSI",0,1, 'C' );
        $fpdf->ln(-8);
        $fpdf->SetX(140);
        ...
        $fpdf->SetX(140);
        $fpdf->Cell(22,6," ",0,0,"L" );
        $fpdf->Cell(43,6," ",0,1,"L" );
        $fpdf->SetX(140);
        $fpdf->Cell(22,6," ",0,0,"L" );
        $fpdf->Cell(2,6,"",0,0,"L" );
        $fpdf->MultiCell(41,6,'',0,'L' );
        $fpdf->SetXY(140,18);
