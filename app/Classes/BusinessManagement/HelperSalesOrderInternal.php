...
class HelperSalesOrderInternal
{
    public static function CancelSO($id)
    {
        $user = Auth::user()->user_name;
        $so = SalesOrderInternal::find($id);
        if ($so != null) {
            try {
                DB::beginTransaction();
    ...
    public static function cetakPdfSO($data)
    {
        ...
        $txtTerm = "";
        if ($dataSalesOrderInternal->metode_pembayaran == "cash") {
            $txtTerm = strtoupper($dataSalesOrderInternal->metode_pembayaran);
        }
        else {
            $txtTerm = strtoupper($dataSalesOrderInternal->metode_pembayaran)." ".$dataSalesOrderInternal->durasi_jt." Hari";
        }
        $txtDiskon = "";
        $nominalDiskon = $dataSalesOrderInternal->nominal_so_dpp * ($dataSalesOrderInternal->persentase_diskon / 100);
        if ($dataSalesOrderInternal->persentase_diskon > 0)
        {
        ...
        $txtPPn = "";
        if ($dataSalesOrderInternal->nominal_so_ppn > 0)
        {
            $txtPPn = number_format(($dataSalesOrderInternal->nominal_so_ppn),2,",",".");
        }
        else {
            $txtPPn = "-";
        }
        ...
        {
            $txtDp = number_format($dataSalesOrderInternal->nominal_dp, 0, ',', '.');
        }
        else {
            $txtDp = "-";
        }
        ...
        if ($dataSalesOrderInternal->metode_pembayaran == "cash") {
            ...
            $txtTerm = "Kredit ".$dataSalesOrderInternal->durasi_jt." Hari";
        }

        $nominalTerbilang = $dataSalesOrderInternal->nominal_so_ttl - $dataSalesOrderInternal->nominal_dp;
        $txtTerbilang = Helper::number_to_words($nominalTerbilang);
        $txtTerbilang = "#".ucwords($txtTerbilang)." Rupiah";

        //header sales order
        $fpdf->AddPage();
        $fpdf->SetTitle(strtoupper($dataSalesOrderInternal->no_so));
        ...
        $fpdf->Cell(40,4,": ".strtoupper($dataSalesOrderInternal->no_so),0,1,"L" );
        $fpdf->Cell(40,4," Tanggal SO ",0,0,"L" );
        $fpdf->Cell(40,4,": ".date("d M Y", strtotime($dataSalesOrderInternal->tanggal_so)),0,1,"L" );
        $fpdf->Cell(40,4," Tanggal Perkiraan Kirim ",0,0,"L" );
        $fpdf->Cell(40,4,": ".date("d M Y", strtotime($dataSalesOrderInternal->tanggal_request)),0,1,"L" );
        $fpdf->Cell(40,4,"",0,1,"L" );
        $fpdf->ln(-21);
        ...
        $fpdf->SetX(91);
        $fpdf->Cell(115,5,strtoupper($dataSalesOrderInternal->nama_customer).$outlet,0,1, 'L' );
        $fpdf->SetFont('Arial','',9);
        $fpdf->SetX(91);
        $fpdf->MultiCell(110,4,$alamat,0,'L' );
        $fpdf->SetXY(91,33);
        $fpdf->Cell(25,4,"Telp ",0,0,"L" );
        $fpdf->Cell(90,4,": ".$dataSalesOrderInternal->telp_customer,0,1,"L" );
        $fpdf->SetX(91);
        ...
        $fpdf->Cell(90,4,": ".strtoupper($dataSalesOrderInternal->no_po_customer),0,1,"L" );
        $fpdf->SetX(91);
        ...
        $nmr = 1;
        foreach ($detailSalesOrderInternal as $dataItem) {
            ...
            if ($dataSalesOrderInternal->flag_ppn == "I") {
        ...
        $fpdf->Cell(32,5,number_format(($dataSalesOrderInternal->nominal_so_dpp),2,",","."),0,1,"R" );
        $fpdf->SetX(128);
        $fpdf->Cell(35,5,"DISKON ".$dataSalesOrderInternal->persentase_diskon."%",0,0,"L" );
        $fpdf->Cell(10,5,"Rp",0,0,"C" );
        ...
        if ($dataSalesOrderInternal->nominal_dp == 0) {
            $fpdf->Cell(32,5,number_format(($dataSalesOrderInternal->nominal_so_ttl),2,",","."),"T",1,"R" );
        }
        else {
            $fpdf->Cell(32,5,number_format(($dataSalesOrderInternal->nominal_so_ttl - $dataSalesOrderInternal->nominal_dp),2,",","."),"T",1,"R" );
        }
        ...
        if($dataSalesOrderInternal->status_invoice == "draft"){
    ...
    public static function cetakPdfInvDP($data)
    {
        ...
        if ($dataSalesOrderInternal->jenis_diskon == "P") {
            $nominalDiskon = $dataSalesOrderInternal->nominal_so_dpp * ($dataSalesOrderInternal->persentase_diskon / 100);
            if ($dataSalesOrderInternal->persentase_diskon > 0)
            {
        ...
        elseif ($dataSalesOrderInternal->jenis_diskon == "N") {
            $nominalDiskon = $dataSalesOrderInternal->nominal_diskon;
            if ($dataSalesOrderInternal->nominal_diskon > 0)
            {
                $txtDiskon = number_format($nominalDiskon,2,",",".");
            }
            else {
        ...
        if ($dataSalesOrderInternal->nominal_so_ppn > 0)
        {
            $txtPPn = number_format($dataSalesOrderInternal->nominal_so_ppn);
        }
        else {
            $txtPPn = "-";
        }
        ...
        if ($dataSalesOrderInternal->metode_pembayaran == "cash") {
            ...
            $txtTerm = "Kredit ".$dataSalesOrderInternal->durasi_jt." Hari (".date("d M Y", strtotime($dataSalesOrderInternal->tanggal_jt)).")";
        }

        $no_inv = str_replace("so", "invdp", $dataSalesOrderInternal->no_so);
        ...
        $fpdf->Cell(35,5,"SUBTOTAL",0,0,"L" );
        $fpdf->Cell(10,5,"Rp",0,0,"C" );
        $fpdf->Cell(32,5,number_format($dataSalesOrderInternal->nominal_so_dpp),0,1,"R" );
        $fpdf->SetX(128);
        if ($dataSalesOrderInternal->persentase_diskon == "P") {
            $fpdf->Cell(35,5,"DISKON ".$dataSalesOrderInternal->persentase_diskon."%",0,0,"L" );
        }
        else {
        ...
        $fpdf->Cell(10,5,"Rp","T",0,"C" );
        if ($dataSalesOrderInternal->nominal_dp == 0) {
            $fpdf->Cell(32,5,number_format($dataSalesOrderInternal->nominal_so_ttl),"T",1,"R" );
        }
        else {
            $fpdf->Cell(32,5,number_format($dataSalesOrderInternal->nominal_so_ttl - $dataSalesOrderInternal->nominal_dp),"T",1,"R" );
        }
        //end of blok grand total
    ...
    public static function cetakPdfInvPelunasan($data)
    {
        ...
        $no_inv = str_replace("so", "PINV", $dataSalesOrderInternal->no_so);
        ...
        //Blok Detail Alamat
        $fpdf->SetFont('Arial','B',9);
        $fpdf->Cell(65,5,$dataSalesOrderInternal->nama_customer.$outlet,0,1, 'L' );
        $fpdf->SetFont('Arial','',9);
        ...
        $fpdf->SetXY(75,36);
        $fpdf->SetFont('Arial','B',9);
        $fpdf->Cell(65,5,$dataSalesOrderInternal->nama_customer.$outletTagih,0,1, 'L' );
        $fpdf->SetFont('Arial','',9);
        $fpdf->SetX(75);
        ...
        $fpdf->Cell(41,5.5,": ".Carbon::parse($dataSalesOrderInternal->tanggal_so)->isoFormat('D MMMM Y'),0,1,"L" );
        $fpdf->SetX(140);
        $fpdf->Cell(24,5.5," Perkiraan Kirim ",0,0,"L" );
        $fpdf->Cell(41,5.5,": ".Carbon::parse($dataSalesOrderInternal->tanggal_request)->isoFormat('D MMMM Y'),0,1,"L" );
        $fpdf->SetX(140);
        $fpdf->Cell(24,5.5," No. PO ",0,0,"L" );
        $fpdf->Cell(2,5.5,":",0,0,"L" );
        $fpdf->MultiCell(41,5.5,strtoupper($dataSalesOrderInternal->no_po_customer),0,'L' );
        $fpdf->SetX(140);
        ...
        if ($dataSalesOrderInternal->metode_pembayaran == "cash") {
            ...
            $fpdf->Cell(41,5.5,": Kredit ".$dataSalesOrderInternal->durasi_jt." Hari",0,1,"L" );
        ...
        $nmr = 1;
        $fpdf->SetFont('Arial','',9);
        $countItem = count($detailSalesOrderInternal);
        if($countItem > 0){
            foreach ($detailSalesOrderInternal as $dataItem) {
        ...
        $fpdf->Cell(32,5,$txtDiskon,0,1,"R" );
        $fpdf->SetX(128);
        if ($dataSalesOrderInternal->ppn > 0){
        ...
        $fpdf->SetFont('Arial','',9);
        $fpdf->SetX(165);
        $fpdf->Cell(35,5,"Tangerang, ".Carbon::parse($dataSalesOrderInternal->tanggal_so)->isoFormat('D MMMM Y'),0,1,"R" );
        $fpdf->SetX(165);
