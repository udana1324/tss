...
class ProductSpecialPricingController extends Controller
{
    public function index()
    {
        if (Auth::check()) {
            ...
            if ($countAkses > 0) {
                ...
                $dataStatus = ProductSpecialPricing::distinct()->get('status_quotation');

                $delete = DB::table('product_special_pricing_detail')
                                ->where([
                                    ['deleted_at', '!=', null],
                ...
                $log = ActionLog::create([
                    'module' => 'Product Special Pricing',
                    'action' => 'Tampil',
                    'desc' => 'Tampilan Product Special Pricing',
                    'username' => Auth::user()->user_name
                ]);

                return view('pages.sales.product_special_pricing.index', $data);
            }
            else {
