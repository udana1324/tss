...
class SalesOrderController extends Controller
{
    public function index()
    {
        if (Auth::check()) {
            ...
            if ($countAkses > 0) {
                ...
                $dataStatus = SalesOrder::distinct()->get('status_so');
                ...
                return view('pages.sales.sales_order.index', $data);
            }
            else {
    ...
    public function getDataIndex(Request $request)
    {
        ...
        $salesOrder = SalesOrder::leftJoin('customer', 'sales_order.id_customer', '=', 'customer.id')
                            ...
                            ->orderBy('sales_order.no_so', 'desc')
                            ->get();
    ...
    public function create()
    {
        if (Auth::check()) {
            ...
            if ($hakAkses->add == "Y") {
                ...
                $delete = DB::table('sales_order_detail')
                            ->where([
                                ['id_so', '=', 'DRAFT'],
                ...
                return view('pages.sales.sales_order.add', $data);
            }
            else {
        ...
        if (Auth::check()) {
            ...
            if ($hakAkses->edit == "Y") {
                ...
                if ($dataSalesOrder->status_so != "draft") {
                    return redirect('/SalesOrder')->with('warning', 'Sales Order tidak dapat diubah karena status Penjualan bukan DRAFT!');
                }
                ...
                if ($dataSalesOrder->status_so != "draft") {
                    return redirect('/SalesOrder')->with('warning', 'Sales Order tidak dapat diubah karena status Sales Order bukan DRAFT!');
                }
                ...
                $dataDetail = SalesOrderDetail::where([
                                                    ['id_so', '=', $id]
                                                ])
                                                ->get();
    ...
    public function detail($id)
    {
        if (Auth::check()) {
            ...
            if ($hakAkses != null) {
                ...
                $dataSalesOrder = SalesOrder::find($id);
                $dataCustomer = Customer::withTrashed()->find($dataSalesOrder->id_customer);
                $dataTerms = SalesOrderTerms::where('id_so', $id)->get();
                $dataAlamat = CustomerDetail::find($dataSalesOrder->id_alamat);
                ...
                $parentMenu = Module::find($hakAkses->parent);
                $taxSettings = TaxSettingsPPN::find($dataSalesOrder->id_ppn);
                ...
                return view('pages.sales.sales_order.detail', $data);
            }
            else {
                return redirect('/SalesOrder')->with('warning', 'Anda tidak memiliki Hak Akses untuk Halaman tersebut!');
            }
    ...
    public function getDataItem(Request $request)
    {
        ...
        if ($idProduct != "" && $idSatuan != "") {
            ...
            $hargaJualTerakhir = SalesOrderDetail::leftJoin('sales_order', 'sales_order_detail.id_so', '=', 'sales_order.id')
                                                    ...
                                                    ->whereIn('sales_order.tanggal_so', function($querySub) use ($idProduct, $idCustomer, $idSatuan, $idAlamat) {
                                                        $querySub->select(DB::raw("MAX(sales_order.tanggal_so)"))->from("sales_order")
                                                                ...
                                                                ->where([
                                                                    ...
                                                                    ['sales_order.id_alamat', '=', $idAlamat],
                                                    ...
                                                    $q->whereIn('sales_order.id_customer', $DataGroupCustomer);
                                                })
                                                ->when($isInGroup == 0, function($q) use ($idCustomer) {
                                                    $q->where('sales_order.id_customer', '=', $idCustomer);
                                                })
                                                ->where([
                                                        // ['sales_order.id_customer', '=', $idCustomer],
                                                        ['sales_order_detail.id_satuan', '=', $idSatuan],
    ...
    public function StoreSalesOrderDetail(Request $request)
    {
        $data = "";
        $exception = DB::transaction(function () use ($request, &$data) {
            $id = $request->input('idSo');
            ...
            if ($id == "") {
                $id = 'DRAFT';
                $countItem = DB::table('sales_order_detail')
    ...
    public function UpdateSalesOrderDetail(Request $request)
    {
        ...
        $exception = DB::transaction(function () use ($request, &$data) {
            ...
            if ($id == "") {
                $id = 'DRAFT';
                $listItem = SalesOrderDetail::find($idDetail);
                $listItem->id_so = $id;
                $listItem->id_item = $idItem;
                $listItem->id_satuan = $idSatuan;
                $listItem->qty_item = $qty;
    ...
    public function GetSalesOrderDetail(Request $request)
    {
        ...
        if ($mode != "edit") {
            ...
            $detail = SalesOrderDetail::leftJoin('product', 'sales_order_detail.id_item', '=', 'product.id')
                                        ->leftJoin('product_unit', 'sales_order_detail.id_satuan', 'product_unit.id')
                                        ->leftJoinSub($dataSpek, 'dataSpek', function($dataSpek) {
                                            $dataSpek->on('product.id', '=', 'dataSpek.id_product');
                                        })
                                        ...
                                        ->when($id == "DRAFT", function($q) use ($user) {
                                            $q->where('sales_order_detail.created_by', $user);
                                        })
                                        ->get();
        }
        else {
            ...
            $detail = TempTransaction::leftJoin('product', 'temp_transaction.value2', '=', 'product.id')
                                        ...
                                        ->where([
                                            ...
                                            ['temp_transaction.module', '=', 'sales_order']
                                        ])
                                        ->get();
        }
    ...
    public function EditSalesOrderDetail(Request $request)
    {
    ...
    public function DeleteSalesOrderDetail(Request $request)
    {
        $data = "";
        $exception = DB::transaction(function () use ($request, &$data) {
            $id = $request->input('idDetail');
            ...
            if ($mode != "") {
                ...
                $delete = DB::table('sales_order_detail')->where('id', '=', $id)->delete();
            }
        });
    ...
    public function GetSalesOrderFooter(Request $request)
    {
        $id = $request->input('idSo');
        $mode = $request->input('mode');
        $user = Auth::user()->user_name;
        ...
        if($mode != "edit") {
            $detail = SalesOrderDetail::leftJoin('product', 'sales_order_detail.id_item', '=', 'product.id')
                                        ->leftJoin('product_unit', 'sales_order_detail.id_satuan', 'product_unit.id')
                                        ->select(
                                            DB::raw('SUM(sales_order_detail.qty_item) AS qtyItem'),
                                            DB::raw('SUM(COALESCE(sales_order_detail.harga_jual,0) * COALESCE(sales_order_detail.qty_item,0)) AS subtotal')
                                        )
                                        ->where([
                                            ['sales_order_detail.id_so', '=', $id]
                                        ])
                                        ...
                                        ->groupBy('sales_order_detail.id_so')
                                        ->first();
        }
        else {
            //Legend
    ...
    public function store(Request $request)
    {
        ...
        if (!$aksesTransaksi) {
            return redirect('/SalesOrder')->with('danger', 'Transaksi gagal!. Akses Transaksi Bulan '.$bulanIndonesia.' belum dibuka!');
        }

        //CekPeriodePPN
        $periodePPN = Helper::CheckPPNPeriod($tglSo);
        if (!$periodePPN && $flagPpn != "N") {
            return redirect('/SalesOrder')->with('danger', 'Transaksi gagal!. Transaksi Diluar periode PPn, silahkan update Pengaturan Faktur Pajak Terlebih Dahulu!');
        }
        ...
        $exception = DB::transaction(function () use ($request, &$data) {
            ...
            $countKode = DB::table('sales_order')
            ...
            if ($counter < 10) {
                $nmrSo = "so-cv-".$kodeTgl."0".$counter;
            }
            else {
                $nmrSo = "so-cv-".$kodeTgl.$counter;
            }
            ...
            if ($file != "") {
                ...
                $namaFile = $nmrSo.".".$ext;
                $request->file('file_po_customer')->storeAs('sales_order/', $namaFile, 'documents');
            }
            else {
                $namaFile = "";
            }
            ...
            $setDetail = DB::table('sales_order_detail')
                            ->where([
                                        ['id_so', '=', 'DRAFT'],
                                        ['created_by', '=', Auth::user()->user_name]
                                    ])
            ...
            if ($terms != "") {
                ...
                salesOrderTerms::insert($listTerms);
            }
        ...
        if (is_null($exception)) {
            return redirect()->route('SalesOrder.Detail', [$data->id])->with('success', 'Data '.strtoupper($data->no_so).' Telah Disimpan!');
        }
        else {
            return redirect('/SalesOrder')->with('error', $exception);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $aksesTransaksi = Helper::cekAksesPeriode($tglSo);
        if (!$aksesTransaksi) {
            return redirect()->route('SalesOrder.edit', [$id])->with('danger', 'Transaksi gagal!. Akses Transaksi Bulan '.$bulanIndonesia.' belum dibuka!');
        }
        ...
        $exception = DB::transaction(function () use ($request, $id, &$data) {
            ...
            if ($updateFile != "") {
                Storage::disk('documents')->delete('sales_order/'.$updateFile);
            ...
            if ($tempDetail != "") {
                foreach ($tempDetail as $detail) {
                    if ($detail->action == "update") {
                        $listItem = SalesOrderDetail::find($detail->id_detail);
                        ...
                        $delete = DB::table('sales_order_detail')->where('id', '=', $detail->id_detail)->delete();
                    }
                }
            }
        ...
        if (is_null($exception)) {
            return redirect()->route('SalesOrder.Detail', [$data->id])->with('success', 'Data '.strtoupper($data->no_so).' Telah Diupdate!');
        }
        else {
            return redirect('/SalesOrder')->with('error', $exception);
        }
    ...
    public function posting(Request $request, $id)
    {
        ...
        $exception = DB::transaction(function () use ($request, $id, &$data, &$msg, &$status) {
            ...
            elseif ($btnAction == "revisi") {
                if ($cekSjPosted == 0) {
                    ...
                    $msg = 'Sales Order '.strtoupper($salesOrder->no_so).' Tidak dapat Direvisi karena terdapat Surat Jalan Pengiriman atas Sales Order '.strtoupper($salesOrder->no_so).' !';
                    $status = 'warning';
                }
            }
        ...
        if (is_null($exception)) {
            if ($status == "ubah") {
                return redirect()->route('SalesOrder.edit', [$id]);
            }
            elseif($status == "kirim") {
    ...
    public function ResetSalesOrderDetail(Request $request)
    {
        $exception = DB::transaction(function () use ($request) {
            $id = $request->input('idSO');
            ...
            if ($id != "DRAFT") {
                ...
                $delete = DB::table('sales_order_detail')->where('id_so', '=', $id)->delete();
            }
        });
    ...
    public function SetSalesOrderDetail(Request $request)
    {
        $data = new stdClass();
        $exception = DB::transaction(function () use ($request, &$data) {
            $id = $request->input('idSo');
            ...
            SalesOrderDetail::insert($listDetail);
        });
