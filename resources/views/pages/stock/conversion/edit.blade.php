			...
			<div class="content">
				...
				<div class="card card-custom card-sticky">
					<div class="card-header bg-primary header-elements-sm-inline">
						<h6 class="card-title text-white">Ubah Konversi Stok Barang {{strtoupper($dataConversion->kode_transaksi)}}</h6>
					</div>
                    <form action="{{ route('StockConversion.update', $dataConversion->id) }}" class="form-horizontal" id="form_add" method="POST">
					    <div class="card-body">
                            {{ csrf_field() }}
                            @method('PUT')
                            <div class="row">
								...
								<div class="col-md-6">
									<fieldset>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-12">
                                                <textarea class="form-control" id="keterangan" name="keterangan" rows="3" placeholder="Ketik Keterangan Disini">{{$dataConversion->keterangan}}</textarea>
                                            </div>
                                        </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#tanggal_conversion_picker").datepicker("setDate", new Date("{{$dataConversion->tgl_transaksi}}"));
        });
                            ...
                            success: function(result){
                                if (result == "success") {
                                    ...
                                    var datatableFrom = $('#list_item_from').KTDatatable();
                                        datatableFrom.setDataSourceParam('idConversion', '{{$dataConversion->id}}');
                                        datatableFrom.reload();

                                    var datatableTo = $('#list_item_to').KTDatatable();
                                        datatableTo.setDataSourceParam('idConversion', '{{$dataConversion->id}}');
                                        datatableTo.reload();
                                }
        ...
        $("#cancel").on('click', function(e) {
            Swal.fire({
                title: "Batal?",
                text: "Apakah anda ingin membatalkan perubahan data konversi?",
                icon: "warning",
                showCancelButton: true,
                confirmButtonText: "Ya",
                cancelButtonText: "Tidak",
                ...
                if(result.value) {
                    ...
                    $.ajax({
                        url: "/StockConversion/RestoreDetail",
                        method: 'POST',
                        data: {
                            idConversion: '{{$dataConversion->id}}'
                        },
