			...
			<div class="content">
				...
				<div class="card card-custom">
					<div class="card-header bg-primary header-elements-sm-inline">
						<h5 class="card-title text-white">Ubah Pengiriman</h5>
					</div>
                    <form action="{{ route('Delivery.update', $dataDlv->id) }}" class="form-horizontal" id="form_add" method="POST">
					    <div class="card-body">
                            {{ csrf_field() }}
                            @method('PUT')
                            <div class="row">
									...
									<fieldset>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="form-group form-group-feedback form-group-feedback-right">
                                                <input type="text" class="form-control" id="po_pelanggan" readonly>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="form-group form-group-feedback form-group-feedback-right">
                                                ...
                                                <input type="text" class="form-control" placeholder="Pilih Tanggal" name="tanggal_sj_picker" id="tanggal_sj_picker">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div>
                                                <div class="radio-inline">
                                                    <label class="radio">
                                                        <input type="radio" id="termsSo" value="use" name="terms_usage" {{ $dataDlv->flag_terms_so == 1 ? "checked" : "" }} />
                                                        <span></span>Sales Order
                                                    </label>
                                                    <label class="radio">
                                                        <input type="radio" id="buatTerms" value="new" name="terms_usage" {{ $dataDlv->flag_terms_so == 0 ? "checked" : "" }} />
                                                        <span></span>Buat Baru
                                                    </label>
                                                </div>
                                            </div>
                                        ...
                                        <div class="form-group" id="divTnc">
                                            ...
                                            <div class="form-group form-group-feedback form-group-feedback-right">
                                                <div class="input-group">
                                                    <textarea class="form-control elastic" id="tnc" name="tnc" rows="3" {{ $dataDlv->flag_terms_so == 1 ? "readonly" : "" }} placeholder="Ketik Syarat & Ketentuan Pengiriman Disini atau gunakan Template pada tombol Template">@foreach($dataTerms as $terms){{ucwords($terms->terms_and_cond)}}@endforeach</textarea>
                                                    <div class="input-group-append">
                                                        <button type="button" class="btn btn-primary" id="btnTemplate" {{ $dataDlv->flag_terms_so == 1 ? "disabled" : "" }} data-toggle="modal" data-target="#modal_list_terms">Template</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
								...
								<div class="col-md-6">
									<fieldset>
                                        ...
                                        <div class="form-group row">
                                            <div class="col-6">
                                                <label>Jumlah Sisa Pesanan :</label>
                                                <div class="input-group">
                                                    <div class="col-12">
                                                        <input type="text" id="qtyOuts" autocomplete="off" data-a-dec="," data-a-sep="." class="form-control text-right" readonly>
                                                    </div>
                                                    {{-- <div class="col-4">
...
@section('scripts')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#qtyItemMask").autoNumeric('init', {mDec: '0'});

            $("#customer").val("{{$dataDlv->id_customer}}").trigger("change");
            $("#tanggal_sj").val('{{$dataDlv->tanggal_sj}}');
        ...
        $("#cancel").on('click', function(e) {
            Swal.fire({
                title: "Keluar?",
                text: "Apakah anda ingin membatalkan perubahan pengiriman barang?",
                icon: "warning",
                ...
                $.ajax({
                    ...
                    success: function(result){
                        var datatable = $('#list_item').KTDatatable();
                            ...
                            datatable.setDataSourceParam('idDelivery', '{{$dataDlv->id}}');
        ...
        $('#salesOrder').on('change', function() {
            ...
            if ($("#mode").val() == "edit") {
                ...
                if ($(this).val() != "") {
                    ...
                    $.ajax({
                        url: "/Delivery/GetDefaultAddress",
                        method: 'POST',
                        data: {
                    ...
                    $.ajax({
                        ...
                        success: function(result){
                            if (result.length != "") {
                                var today = new Date();
                                var reqDate = new Date(result.tanggal_request);
                                var selisih = Math.floor((Date.UTC(reqDate.getFullYear(), reqDate.getMonth(), reqDate.getDate()) - Date.UTC(today.getFullYear(), today.getMonth(), today.getDate()) ) /(1000 * 60 * 60 * 24));
                                if (selisih < 0) {
                                    $("#tanggal_sj_picker").val("");
                                    $("#tanggal_sj").val("");
                                }
                                else {
                                    $("#tanggal_sj").val(result.tanggal_request);
                                    $("#tanggal_sj_picker").datepicker('setDate', new Date(result.tanggal_request));
                                }
                                if (result.metode_kirim == "delivery") {
                                    $("#metode_pengiriman").val("Kirim");
                                }
                                else if (result.metode_kirim == "pickup") {
                                    $("#metode_pengiriman").val("Ambil Sendiri");
                                }
                                else if (result.metode_kirim == "ekspedisi") {
                                    $("#metode_pengiriman").val("Ekspedisi");
                                }

                                $("#po_pelanggan").val(result.no_po_customer);
                            }
                        }
                    });
        ...
        $("#product").on("change", function() {
            ...
            $.ajax({
                ...
                data: {
                    idProduct: $(this).val(),
                    idSo: $("#salesOrder option:selected").val(),
                },
                success: function(result){
        ...
        function getProduct(idSo) {
            ...
            $.ajax({
                url: "/Delivery/GetProductBySalesOrder",
                method: 'POST',
                data: {
                    idSalesOrder: idSo,
                },
        ...
        function getCustomerSalesOrder(idCustomer) {
            ...
            $.ajax({
                ...
                success: function(result){
                    ...
                    if ($("#mode").val() == "load") {
                        $("#salesOrder").val("{{$dataDlv->id_so}}").trigger("change");
                        $("#mode").val("edit");
                        var datatable = $('#list_item').KTDatatable();
                            datatable.setDataSourceParam('idSalesOrder', $("#salesOrder").val());
                            datatable.setDataSourceParam('idDelivery', '{{$dataDlv->id}}');
                            datatable.setDataSourceParam('mode','edit');
                            datatable.reload();
                    }
                        ...
                        $.ajax({
                            ...
                            success: function(result){
                                if (result == "success") {
                                    ...
                                    $("#qtyOuts").val("");
                                    var datatable = $('#list_item').KTDatatable();
                                        datatable.setDataSourceParam('idSalesOrder', $("#salesOrder option:selected").val());
                                        datatable.setDataSourceParam('idDelivery','{{$dataDlv->id}}');
                                        datatable.setDataSourceParam('mode','edit');
                                        datatable.reload();
                                    footerDataForm('{{$dataDlv->id}}');
            ...
            $.ajax({
                ...
                success: function(result){
                    if (result.length > 0) {
                        ...
                        var qtyOutstanding = parseFloat(result[0].qty_outstanding);
                        ...
                        var data = "<tr>";
                            ...
                            data += "<td style='text-align:center;'><input type='text' class='form-control' id='qtyRowEditMask' autocomplete='off' data-a-dec=',' data-a-sep='.' value='"+qty+"' /></td>";
                            data += "<td style='text-align:center;'><input type='hidden' class='form-control inputEdit' id='qtyRowEdit' value='"+qty+"' /></td>";
                            data += "</tr>";
                            $('#detil_edit_item').append(data);

                            $("#qtyRowEditMask").autoNumeric('init', {mDec: '0'});
