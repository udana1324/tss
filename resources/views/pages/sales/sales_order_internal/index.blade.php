				...
				<div class="card card-custom">
					<div class="card-header bg-primary header-elements-sm-inline">
                        ...
                        <div class="card-toolbar">
                            <!--begin::Button-->
                            @if($hakAkses->add == "Y")
                            <button class="btn btn-primary btn-outline-white mr-2" onclick="window.location.href = '{{ url('SalesOrderInternal/Add') }}';">
					...
					<div class="card-body">
                        <form action="{{ route('SalesOrderInternal.Export') }}" class="form-horizontal" id="form_add" method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <!--begin::Search Form-->
                        <div class="mb-7">
		...
		$(document).ready(function() {
            ...
            var datatable = $('#table_so').KTDatatable({
                data: {
                    ...
                    source: {
                        read: {
                            url: '/SalesOrderInternal/GetData',
                            method: 'GET',
                            headers : {
                    ...
                    },
                    {
                        ...
                        template: function(row) {
                            ...
                            var txtAction = '<div class="dropdown dropdown-inline">';
                                ...
                                txtAction += "<li class='nav-item'><a class='nav-link' href='{{route('SalesOrderInternal.Detail', 'idSo')}}'><i class='nav-icon la la-eye'></i><span class='nav-text'>Detail</span></a></li>";
                                if (akses.edit == "Y" && row.status_so == "draft") {
                                    txtAction += "<li class='nav-item'><a class='nav-link' href='{{route('SalesOrderInternal.edit', 'idSo')}}'><i class='nav-icon la la-edit'></i><span class='nav-text'>Ubah</span></a></li>";
                                }
                                if (akses.delete) {
        ...
        function deleteData(id) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/SalesOrderInternal/Delete",
                        method: 'POST',
                        data: {
                            idSalesOrderInternal: id
                        },
