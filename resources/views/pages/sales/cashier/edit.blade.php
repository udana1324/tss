                ...
                <div class="flex-row-fluid ml-lg-8">
                    ...
                    <div class="card card-custom">
                        ...
                        <div class="card-footer bg-white d-sm-flex justify-content-sm-between align-items-sm-center">
                            ...
                            <div class="mt-2 mt-sm-0">
                                <button type="button" id="btnReset" class="btn btn-light-primary font-weight-bold mr-2" style="display:none ;"> Refresh </button>
                                <button type="button" id="btnCetak" class="btn btn-outline-primary" style="display:none ;">Cetak Receipt<i class="fas fa-print ml-2"></i></button>
...
@section('scripts')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#customer").val("{{$dataTransaction->id_customer}}").trigger("change");
            loadItems();
            ...
            $.ajax({
                ...
                success: function(result){
                    if (result.message == "success") {
                        ...
                        window.location.href = "{{ route('Cashier.Detail', 'idTransaction') }}".replace('idTransaction', result.id);
        ...
        function loadItems() {
            var tableData = @json($detail);
            for (var i = 0;i < tableData.length;i++) {
                var qty = tableData[i].qty_item;
                var harga = tableData[i].harga_jual;
                var idProduct = tableData[i].id_item;
                var txtProduct = tableData[i].nama_item;
                var idSatuan = tableData[i].id_satuan;
                var txtSatuan = tableData[i].nama_satuan;
                var modelSales = tableData[i].mode;
                var qtyModel = tableData[i].qty_mode;
                var hargaNormal = tableData[i].harga_jual_standard;
                var jenisGrosir = tableData[i].jenis_grosir;
                var hargaGrosir = tableData[i].harga_grosir;
                var qtyGrosir = tableData[i].qty_grosir;
                var jenisGrosir2 = tableData[i].jenis_grosir_2;
                var hargaGrosir2 = tableData[i].harga_grosir_2;
                var qtyGrosir2 = tableData[i].qty_grosir_2;
                var txtHarga = parseFloat(harga).toLocaleString('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0});
                var txtQty = parseFloat(qty).toLocaleString('id-ID', { minimumFractionDigits: 0, maximumFractionDigits: 0});
        ...
        function loadDetails() {
            var metodePembayaran = "{{$dataTransaction->metode_pembayaran}}";

            if (metodePembayaran == "edc") {
				showHidePaymentDetails("show", "carddetail");
                showHidePaymentDetails("hide", "cashdetail");
                showHidePaymentDetails("hide", "rekeningdetail");
                ...
                $("#ccnumber").val("{{$dataTransaction->cc_number}}");
                $("#ccname").val("{{$dataTransaction->cc_name}}");
                $("#ccmonth").val("{{$dataTransaction->cc_month}}");
                $("#ccyear").val("{{$dataTransaction->cc_year}}");
			}
                ...
                $("#rekening").val("{{$dataTransaction->id_rekening}}").trigger('change');
			}
                ...
                $("#cash").trigger('click');
                $("#cash_payment_mask").autoNumeric('set', "{{$dataTransaction->nominal_pembayaran}}").trigger('change');
			}
            else {
                showHidePaymentDetails("hide", "carddetail");
                showHidePaymentDetails("hide", "cashdetail");
