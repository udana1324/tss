				...
				<div class="card card-custom">
					<div class="card-header bg-primary header-elements-sm-inline">
                        <div class="card-title">
                            <h3 class="card-label text-white">Ringkasan Pembelian berdasarkan Supplier</h3>
                        </div>
                        <div class="card-toolbar">
                            <!--begin::Button-->
                            @if($hakAkses->export == "Y")
                            <button type="button" class="btn btn-success font-weight-bold mr-2" id="btnExport" onclick="ExportExcel('F','table_cust','RekapSupplier','xlsx');"> Export <i class="fas fa-file-excel"></i></button>
                            @endif
                            <!--end::Button-->
                        </div>
					</div>
                                        ...
                                        <div class="col-md-6 my-2 my-md-0">
                                            <div class="align-items-center">
                                                ...
                                                <select class="form-control select2" id="table_cust_search_cust" style="width: 250px;">
                                                    ...
                                                    <option value="{{$rowSupplier->kode_supplier}}">{{strtoupper($rowSupplier->kode_supplier.' - '.$rowSupplier->nama_supplier)}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
...
@section('scripts')
	<script type="text/javascript">
		...
		$(document).ready(function() {
            ...
            var datatable = $('#table_cust').KTDatatable({
                data: {
                    ...
                    source: {
                        read: {
                            url: '/RekapPembelianSupplier/GetDetail',
                            method: 'POST',
                            headers : {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        template: function(row) {
                            if (parseInt(row.JmlPO) == 0 || row.JmlPO == null) {
                                return row.nama_supplier.toUpperCase() + "<br />" + row.kode_supplier.toUpperCase();
                            }
                            else {
                                var txtAction = "<a href='#' data-toggle='modal' data-target='#modal_detail_sales' title='Detail' onclick='viewDetailItem(" + row.id + ");return false;'>";
                                    txtAction += row.nama_supplier.toUpperCase();
                                    txtAction += "</a>";
                                    txtAction += "<br />" + row.kode_supplier.toUpperCase();
                                return txtAction;
                            }
                        },
                    },
                    {
                        ...
                        visible:false,
                        template: function(row) {
                            return row.kode_supplier.toUpperCase() + "<span id='txt_"+row.id+"'>"+row.nama_supplier.toUpperCase()+"</span>";
                        },
                    },
                    {
                        field: 'JmlPO',
                        ...
                        template: function(row) {
                            if (row.JmlPO != null) {
                                return parseFloat(row.JmlPO).toLocaleString('id-ID', { maximumFractionDigits: 2});
                            }
                            else {
                                return '0';
                            }
                    ...
                    },
                    {
                        ...
                        autoHide: true,
                        template: function(row) {
                            if (row.JmlPODiterima != null) {
                                return parseFloat(row.JmlPODiterima).toLocaleString('id-ID', { maximumFractionDigits: 2});
                            }
                            else {
                                return '0';
                            }
                    ...
                    },
                    {
                        ...
                        autoHide: true,
                        template: function(row) {
                            if (row.JmlPOFull != null) {
                                return parseFloat(row.JmlPOFull).toLocaleString('id-ID', { maximumFractionDigits: 2});
                            }
                            else {
                                return '0';
                            }
            ...
            $('#table_cust_search_cust').on('change', function() {
                datatable.search($(this).val(), 'kode_supplier');
            });
        });
        ...
        $(document).ready(function() {
            ...
            var datatable = $('#list_item_detail').KTDatatable({
                data: {
                    ...
                    source: {
                        read: {
                            url: '/RekapPembelianSupplier/GetDetailSupplier',
                            method: 'POST',
                            headers : {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                            },
        ...
        function viewDetailItem(id) {
            ...
            var datatable = $('#list_item_detail').KTDatatable();
                datatable.setDataSourceParam('idSupplier', id);
                datatable.setDataSourceParam('tglStart', $("#tanggal_picker_start").val());
                datatable.setDataSourceParam('tglEnd', $("#tanggal_picker_end").val());
                datatable.reload();
