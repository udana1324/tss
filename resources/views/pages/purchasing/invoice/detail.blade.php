	...
	@include('pages.alerts')
			<div class="content">
                ...
                <div class="card card-custom position-relative overflow-hidden">
                    <form action="{{ route('PurchaseInvoice.Posting', $dataInv->id) }}" class="form-horizontal" id="form_add" method="POST">
                        {{ csrf_field() }}
						...
						<div class="col-md-9">
							<div class="d-flex justify-content-between align-items-md-center flex-column flex-md-row">
								...
								<div class="order-1 order-md-2">
								    <h1 class="display-3 font-weight-bolder text-white">FAKTUR PEMBELIAN</h1><br>
								    ...
								    @endif
								    <br><h3 class="font-weight-bolder text-white">#{{strtoupper($dataInv->no_po)}}</h3>
								</div>
							</div>
						</div>
					</div>
							...
							<div class="row pb-10">
								<div class="col-md-3 border-right-md">
									...
									<div class="font-size-lg font-weight-bold mb-10">
									    <span class="font-weight-boldest">{{strtoupper($dataInv->nama_supplier)}}</span>
									    <br>{{ucwords($dataAlamat->alamat_supplier)}}
									</div> --}}
									<!--end::Invoice To-->
							...
							<div class="row">
									...
									<div class="d-flex flex-column flex-md-row">
										<div class="d-flex flex-column">
											...
											<div class="font-size-lg mt-15">
												<span class="font-weight-boldest mr-15">Catatan Faktur Pembelian:</span><br>
								...
								<div class="col-md-7 pt-md-25">
								    <div class="align-items-center justify-content-between max-w-350px position-relative ml-auto pb-5">
										<div class="font-weight-boldest font-size-h5">Total Pembelian</div>
					    ...
					    <div class="d-flex justify-content-between align-items-center col-10">
                                ...
                                <div class="mt-2 mt-sm-0">
                                    ...
                                    @elseif($dataInv->status_invoice == "posted")
                                        ...
                                        @endif
                                        @if($hakAkses->print == "Y")
										<a type="button" class="btn btn-primary mt-2 mt-sm-0" href='{{route('PurchaseInvoice.Cetak', $dataInv->id)}}' target="_blank">Cetak<i class="fas fa-print ml-2"></i></a>
									@endif
                                    @endif
                                </div>
                            </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        $("#cancel").on('click', function(e) {
            ...
            if (status == "draft") {
                titleTxt = "Batal?";
                txt = "Apakah anda ingin membatalkan posting invoice pembelian?";
            }
            else {
                titleTxt = "Keluar?";
        ...
        $(".btnSubmit").on("click", function(e){
            ...
            if (btn == "batal") {
                Swal.fire({
                    title: "Pembatalan Invoice Pembelian",
                    //text: "Apakah yakin ingin melakukan Pembatalan Invoice Pembelian?",
                    html: "Apakah yakin ingin melakukan Pembatalan Invoice Pembelian?" + "<br /><br />" +
                    ...
                    '<button type="button" id="btn_batalPO"  onclick="batalInvoice(' + "'batalPO'" + ')" class="btn btn-danger mt-2 mt-sm-0">' + 'Batal hingga Purchase Order' + '</button>',
                    icon: "warning",
                    showCancelButton: true,
                ...
                Swal.fire({
                    title: ucwords(btn) + " Invoice Pembelian?",
                    text: "Apakah yakin ingin melakukan " + ucwords(btn) +" Invoice Pembelian?",
                    icon: "warning",
                    showCancelButton: true,
                    confirmButtonText: "Ya",
                    cancelButtonText: "Tidak",
        ...
        function batalInvoice(aksiBatal) {
            ...
            swal.clickConfirm();
            if (aksiBatal == "batalINV" || aksiBatal == "batalPO") {
                $("#form_add").off("submit").submit();
            }
        }
            ...
            var datatable = $('#list_item').KTDatatable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        textAlign: 'center',
                        template: function(row) {
                            var txtAction = "<p class='font-weight-bolder font-size-lg pt-2'>"+row.kode_penerimaan.toUpperCase()+"</p>";
                            return txtAction;
                        },
                    },
                    {
                    ...
                    },
                    {
                        ...
                        template: function(row) {
                            ...
                            var txtAction = "<p class='font-weight-bolder font-size-lg pt-2'>"+parseFloat(subtotalMask).toLocaleString('id-ID', { maximumFractionDigits: 2})+"</p>";
                            return txtAction;
                        },
                    },
                ],
            ...
            var datatable = $('#list_item_detail').KTDatatable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        width: 70,
                        template: function(row) {
                            return parseFloat(row.qty_item).toLocaleString('id-ID', { minimumFractionDigits: 2,maximumFractionDigits: 2});
                        },
                    },
                    {
                        field: 'nama_satuan',
                    ...
                    },
                    {
                        ...
                        textAlign: 'right',
                        template: function(row) {
                            return parseFloat(row.harga_beli).toLocaleString('id-ID', { minimumFractionDigits: 2,maximumFractionDigits: 2});
                        },
                    },
                    {
                        field: 'subtotal_sj',
                        ...
                        textAlign: 'center',
                        template: function(row) {
                            return parseFloat(row.subtotal_sj).toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2});
                        },
                    },
                ],
            });
        ...
        function viewDetailItem(id) {
            ...
            var datatable = $('#list_item_detail').KTDatatable();
                ...
                datatable.setDataSourceParam('idPo', '{{$dataInv->id_po}}');
                datatable.reload();
            ...
            $.ajax({
                ...
                success: function(result){
                    if (result != "null") {
                        ...
                        var persenDiskon = "{{$dataInv->persentase_diskon}}";
                        var diskonNominal = parseFloat(subtotalFixed) * (parseFloat(persenDiskon) / 100);
