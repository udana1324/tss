			...
			<div class="content">
				...
				<div class="card card-custom">
					<div class="card-header bg-primary text-white header-elements-sm-inline">
						<h5 class="card-title font-weight-semibold">Ubah Penerimaan Produksi</h5>
					</div>
                    <form action="{{ route('ProductionReceiving.update', $dataRcv->id) }}" class="form-horizontal" id="form_add" method="POST">
					    <div class="card-body">
                            {{ csrf_field() }}
                            @method('PUT')
                            <div class="row">
								<div class="col-md-6">
									<fieldset>
										<legend class="font-weight-semibold"><i class="la la-clipboard-list"></i> Informasi Pabrik / Vendor</legend>
                                        ...
                                        <div class="form-group row">
                                            <label class="col-lg-3 col-form-label">No. Perintah Produksi :</label>
                                            <div class="col-lg-9">
                                                <select class="form-control req" id="production_order" name="production_order">
                                                    <option label="Label"></option>
                                                </select>
                                                <span class="form-text text-danger err" style="display:none;">*Harap pilih Perintah produksi terlebih dahulu!</span>
                                            </div>
                                        </div>
...
@section('scripts')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#production_order').select2({
                allowClear: true,
                placeholder: "Pilih Perintah Produksi"
            });
        ...
        $("#cancel").on('click', function(e) {
            Swal.fire({
                title: "Batal?",
                text: "Apakah anda ingin membatalkan perubahan penerimaan barang produksi?",
                icon: "warning",
                ...
                if(result.value) {
                    ...
                    $.ajax({
                        url: "/ProductionReceiving/RestoreDetail",
                        method: 'POST',
                        data: {
                            idRcv: '{{$dataRcv->id}}'
                        },
        ...
        $("#supplier").on("change", function() {
            ...
            if ($("#mode").val() == "edit") {
                ...
                $.ajax({
                    url: "/ProductionReceiving/ResetDetail",
                    method: 'POST',
                    data: {
                        idRcv: '{{$dataRcv->id}}',
                    },
                    success: function(result){
                        var datatable = $('#list_item').KTDatatable();
                                ...
                                datatable.setDataSourceParam('idProductionReceiving', '{{$dataRcv->id}}');
        ...
        $("#tanggal_sj_picker").on('change', function() {
            ...
            var poDate = new Date($("#tanggal").val());
            ...
            if ($("#tanggal").val() != "" && $("#tanggal_sj").val() != "") {
                if (selisih < 0) {
                    Swal.fire(
                        "Error!",
                        "Tanggal SJ tidak boleh dibawah dari tanggal Perintah Produksi!.",
                        "warning"
                    )
                    $("#tanggal_sj").val("");
        ...
        $('#production_order').on('change', function() {
            var idProductionOrder = $(this).val();
            if ($(this).val() != "") {
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                $.ajax({
                    url: "/ProductionReceiving/GetTanggalPo",
                    method: 'POST',
                    data: {
                        idProductionOrder: $(this).val(),
                    },
                    success: function(result){
                        if (result != null) {
                            $("#tanggal").val((result.tanggal));
                            $("#tanggal_picker").val(formatDate(result.tanggal));
            ...
            if ($(this).val() != "") {
                ...
                $.ajax({
                    url: "/ProductionReceiving/GetDefaultAddress",
                    method: 'POST',
                    data: {
                        idProductionOrder: $(this).val(),
                    },
            ...
            if ($("#mode").val() == "edit") {
                ...
                $.ajax({
                    url: "/ProductionReceiving/SetDetail",
                    method: 'POST',
                    data: {
                    ...
                    success: function(result){
                        var datatable = $('#list_item').KTDatatable();
                                datatable.setDataSourceParam('idProductionOrder', idProductionOrder);
                                datatable.setDataSourceParam('idProductionReceiving', '{{$dataRcv->id}}');
                                datatable.setDataSourceParam('mode','edit');
                                datatable.reload();
                                footerDataForm('{{$dataRcv->id}}');
        ...
        $("#product").on("change", function() {
            ...
            $.ajax({
                url: "/ProductionReceiving/GetSatuan",
                method: 'POST',
                data: {
                    idProduct: $(this).val(),
                    idPo: $("#production_order option:selected").val(),
                },
                success: function(result){
        ...
        $("#productUnit").on("change", function() {
            ...
            $.ajax({
                url: "/ProductionReceiving/GetDataItem",
                method: 'POST',
                data: {
                    id_product: $("#product option:selected").val(),
                    idProductionOrder: $("#production_order option:selected").val(),
                    id_satuan: $(this).val(),
                },
                success: function(result){
                    if (result.length > 0) {
        ...
        function getProduct(idPo) {
            ...
            $.ajax({
                url: "/ProductionReceiving/GetProductByProductionOrder",
                method: 'POST',
                data: {
                    idProductionOrder: idPo,
                },
                success: function(result){
        ...
        function getSupplierPurchaseOrder(idSupplier) {
            ...
            $.ajax({
                url: "/ProductionReceiving/GetProductionOrder",
                method: 'POST',
                data: {
                    id_supplier: idSupplier,
                },
                success: function(result){
                    $('#production_order').find('option:not(:first)').remove();
                    if (result.length > 0) {
                        for (var i = 0; i < result.length;i++) {
                            $("#production_order").append($('<option>', {
                                value:result[i].id,
                                text:result[i].no_production_order.toUpperCase()
                            }));
                        }
                    }
                    if ($("#mode").val() == "load") {
                        $("#production_order").val("{{$dataRcv->id_po}}").trigger("change");
                        $("#mode").val("edit");
                        var datatable = $('#list_item').KTDatatable();
                            datatable.setDataSourceParam('idProductionOrder', $("#production_order").val());
                            datatable.setDataSourceParam('idProductionReceiving', '{{$dataRcv->id}}');
                            datatable.setDataSourceParam('mode','edit');
                            datatable.reload();
                    }
                    else {
                        $("#production_order").trigger('change');
                    }
                }
            });
        }
        ...
        $(document).ready(function() {
            ...
            var datatable = $('#list_item').KTDatatable({
                data: {
                    ...
                    source: {
                        read: {
                            url: '/ProductionReceiving/GetDetail',
                            method: 'POST',
                            headers : {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                            },
                        ...
                        $.ajax({
                            ...
                            success: function(result){
                                if (result == "success") {
                                    ...
                                    var datatable = $('#list_item').KTDatatable();
                                        datatable.setDataSourceParam('idProductionOrder', $("#production_order option:selected").val());
                                        datatable.setDataSourceParam('idProductionReceiving','{{$dataRcv->id}}');
                                        datatable.setDataSourceParam('mode','edit');
                                        datatable.reload();
                                    footerDataForm('{{$dataRcv->id}}');
        ...
        function editDetailItem(id) {
            ...
            $.ajax({
                url: "/ProductionReceiving/EditDetail",
                method: 'POST',
                data: {
                    idDetail: id,
                    idProductionOrder : $("#production_order option:selected").val(),
        ...
        function deleteDetailItem(id) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/ProductionReceiving/DeleteDetail",
                        method: 'POST',
                        data: {
                            idDetail: id,
                            mode: "edit"
            ...
            if (errCount == 0) {
                Swal.fire({
                    ...
                    if(result.value) {
                        ...
                        $.ajax({
                            url: "/ProductionReceiving/UpdateDetail",
                            method: 'POST',
                            dataType : 'json',
                            data: {
                                idItem : idItem,
        ...
        $("#list_terms").on('click', '.selectTerms', function() {
            ...
            $.ajax({
                url: "/ProductionReceiving/GetTerms",
                method: 'POST',
                data: {
                    idTemplate: id,
                },
