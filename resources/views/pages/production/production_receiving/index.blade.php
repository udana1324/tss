				...
				<div class="card card-custom">
					<div class="card-header bg-primary header-elements-sm-inline">
                        <div class="card-title">
                            <h3 class="card-label text-white">Daftar Penerimaan Produksi</h3>
                        </div>
                        <div class="card-toolbar">
                            <!--begin::Button-->
                            @if($hakAkses->add == "Y")
                            <button class="btn btn-primary btn-outline-white mr-2" onclick="window.location.href = '{{ url('ProductionReceiving/Add') }}';">
					...
					<div class="card-body">
                        <form action="{{ route('ProductionReceiving.Export') }}" class="form-horizontal" id="form_add" method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <!--begin::Search Form-->
                        <div class="mb-7">
		...
		$(document).ready(function() {
            ...
            var datatable = $('#table_rcv').KTDatatable({
                data: {
                    ...
                    source: {
                        read: {
                            url: '/ProductionReceiving/GetData',
                            method: 'GET',
                    ...
                    },
                    {
                        ...
                        template: function(row) {
                            var txt = "";
                                ...
                                txt += "<br />";
                                txt += '<span class="label label-md label-outline-primary label-inline font-weight-bold">'+row.no_production_order.toUpperCase()+'</span>';
                                return txt;
                        },
                    },
                    {
                    ...
                    },
                    {
                        ...
                        template: function(row) {
                            ...
                            var txtAction = '<div class="dropdown dropdown-inline">';
                                ...
                                txtAction += "<li class='nav-item'><a class='nav-link' href='{{route('ProductionReceiving.Detail', 'idRcv')}}'><i class='nav-icon la la-eye'></i><span class='nav-text'>Detail</span></a></li>";
                                if (akses.edit == "Y" && row.status_penerimaan == "draft") {
                                    txtAction += "<li class='nav-item'><a class='nav-link' href='{{route('ProductionReceiving.edit', 'idRcv')}}'><i class='nav-icon la la-edit'></i><span class='nav-text'>Ubah</span></a></li>";
                                }
                                if (akses.delete) {
        ...
        function deleteData(id) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "/ProductionReceiving/Delete",
                        method: 'POST',
                        data: {
                            idProductionReceiving: id
                        },
