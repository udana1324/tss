				...
				<div class="card card-custom">
					<div class="card-header bg-primary header-elements-sm-inline">
                        <div class="card-title">
                            <h3 class="card-label text-white">Daftar Perintah Produksi</h3>
                        </div>
                        <div class="card-toolbar">
                            <!--begin::Button-->
                            @if($hakAkses->add == "Y")
                            <button class="btn btn-primary btn-outline-white mr-2" onclick="window.location.href = '{{ url('ProductionOrder/Add') }}';">
					...
					<div class="card-body">
                        ...
                        <div class="mb-7">
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-3">
                                    <div class=" align-items-center">
                                        <label class="mr-3">Periode Produksi :</label>
		...
		$(document).ready(function() {
            ...
            var datatable = $('#table_po').KTDatatable({
                ...
                columns: [
                    ...
                    {
                        field: 'no_production_order',
                        title: 'Nomor Perintah Produksi',
                        textAlign: 'left',
                        autoHide: false,
                        width: 'auto',
                        template: function(row) {
                            var txt = "";
                            txt += '<span class="font-weight-bold">'+row.no_production_order.toUpperCase()+'</span>';
                            if (row.outstanding_qty != row.jumlah_total && row.outstanding_qty != 0 && row.status != "close") {
                            ...
                            if (row.po_production != null) {
                                txt += '<br><span class="font-weight-bold text-inline text-primary font-size-xs">No. PO : '+row.po_production.toUpperCase()+'</span>';
                            }
                            else {
                                txt += '<br><span class="font-weight-bold text-inline text-primary font-size-xs">No. PO : - </span>';
                            }

                            if (row.flag_internal == 1) {
                                txt += '<br><span class="font-weight-bold text-inline text-primary font-size-xs">Internal</span>';
                            }
                    ...
                    },
                    {
                        ...
                        autoHide: true,
                        template: function(row) {
                            return parseFloat(row.jumlah_total).toLocaleString('id-ID', { maximumFractionDigits: 2});
                        },
                    },
                    {
                    ...
                    {
                        field: 'tanggal',
                        title: 'Tanggal Perintah Produksi',
                        textAlign: 'center',
                        autoHide: false,
                        width: 'auto',
                        template: function(row) {
                            var txt = "";
                                txt += '<span class="font-weight-bold">'+formatDate(row.tanggal)+'</span>';
                                txt += "<br />";
                    ...
                    },
                    {
                        ...
                        template: function(row) {
                            ...
                            var txtAction = '<div class="dropdown dropdown-inline">';
                                ...
                                txtAction += "<li class='nav-item'><a class='nav-link' href='{{route('ProductionOrder.Detail', 'idPo')}}'><i class='nav-icon la la-eye'></i><span class='nav-text'>Detail</span></a></li>";
                                if (akses.edit == "Y" && row.status == "draft") {
                                    txtAction += "<li class='nav-item'><a class='nav-link' href='{{route('ProductionOrder.edit', 'idPo')}}'><i class='nav-icon la la-edit'></i><span class='nav-text'>Ubah</span></a></li>";
                                }
                                if (akses.delete) {
