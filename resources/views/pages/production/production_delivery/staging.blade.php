...
@section('content')
	...
	@include('pages.alerts')
			<div class="content">
                <form action="{{ route('ProductionDelivery.PostStaging', $dataDlv->id) }}" class="form-horizontal" id="form_add" method="POST">
                {{ csrf_field() }}
                    <div class="card card-custom">
                        <div class="card-header bg-primary header-elements-sm-inline">
                            <h5 class="card-title text-white">Detail Pengaturan Pengiriman Barang Produksi</h5>
                        </div>
                            ...
                            <div class="d-flex flex-column flex-xl-row">
                                <div class="flex-column flex-lg-row-auto w-xl-400px">
                                    ...
                                    <div class="card mb-5">
                                        <div class="card-body">
                                            <div class="pb-5 fs-6">
                                                <div class="font-size-h5 font-weight-bolder">Kode Pengiriman :</div>
                                                <div class="font-size-h6 text-dark-50">
                                                    {{strtoupper($dataDlv->kode_pengiriman)}}
                                                    <span class="label label-primary label-inline ml-1">{{strtoupper($dataDlv->status_pengiriman)}}</span>
                                                    {{ $dataDlv->flag_revisi == "1" ? "Revisi" : "" }}
                                                </div>

                                                <div class="font-size-h5 font-weight-bolder mt-5">Vendor / Supplier :</div>
                                                <div class="font-size-h6 text-dark-50">{{strtoupper($dataDlv->nama_supplier)}}</div>

                                                <div class="font-size-h5 font-weight-bolder mt-5">Alamat Vendor / Supplier :</div>
                                                <div class="font-size-h6 text-dark-50">{{strtoupper($dataDlv->alamat_supplier)}}</div>

                                                <div class="font-size-h5 font-weight-bolder mt-5">Tanggal Surat jalan :</div>
                                                <div class="font-size-h6 text-dark-50">{{\Carbon\Carbon::parse($dataDlv->tanggal_sj)->format('d F Y')}}</div>
                                        ...
                                        <div class="card-body" kt-hidden-height="384" style="overflow: hidden; padding-top: 0px; padding-bottom: 0px;">
                                            ...
                                            <div class="row">
                                                <div class="col-12 mt-5">
                                                    <div class="font-size-h5 font-weight-bolder">Alokasi Pengiriman :</div>
                                                    <div class="row mt-1 dataAlokasi">
                                                        <div class="col-lg-6">
                                                            ...
                                                            <span class="form-text text-danger errItem" style="display:none;">*Harap pilih lokasi terlebih dahulu!</span>
                                                        </div>
                                                        <div class="col-lg-3 alokasiQty">
                                                            <input type="text" class="form-control alokasiInput" id="jmlAlokasiMask_{{$details->id_item}}" name="jmlAlokasiMask_{{$details->id_item}}" placeholder="Jumlah kirim" autocomplete="off" data-a-dec="," data-a-sep="." />
...
@section('scripts')
    <script type="text/javascript">
        ...
        $("#cancel").on('click', function(e) {
            Swal.fire({
                title: "Batal?",
                text: "Apakah anda ingin membatalkan posting alokasi pengiriman barang?",
                icon: "warning",
        ...
        $(".btnSubmit").on("click", function(e){
            ...
            var totalAlokasi = 0;
            var ttlPenerimaan = '{{$dataDlv->jumlah_total_sj}}';
            ...
            Swal.fire({
                title: ucwords(btn) + " Pengiriman Barang Produksi?",
                text: "Apakah yakin ingin melakukan " + ucwords(btn) +" Pengiriman Barang Produksi?",
                icon: "warning",
                showCancelButton: true,
                confirmButtonText: "Ya",
                cancelButtonText: "Tidak",
        ...
        $(".addItem").on('click', function(e) {
            ...
            if (idIndex == "" || idIndex == null) {
                Swal.fire(
                    "Gagal!",
                    "Harap pilih lokasi terlebih dahulu!",
                    "warning"
                );
                return false;
            }
        ...
        $(document).ready(function() {
            var statusDlv = "{{$dataDlv->status_pengiriman}}";
            if (statusDlv != "posted") {
