				...
				<div class="card card-custom">
					<div class="card-header bg-primary header-elements-sm-inline">
                        <div class="card-title">
                            <h3 class="card-label text-white">Daftar Pembayaran Grup Pelanggan</h3>
                        </div>
					</div>
                            ...
                            <div class="row align-items-center">
                                <div class="col-lg-10">
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-lg-3" id="divGrup">
                                            <div class="align-items-center">
                                                <label>Grup :</label>
                                                <select class="form-control select2" id="id_grup" name="id_grup" style="width: 100%;">
                                                    <option value="">Semua Grup</option>
                                                    @foreach($dataGroup as $group)
                                                    <option value="{{$group->nama_group}}">{{strtoupper($group->nama_group)}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
		...
		$(document).ready(function() {
            ...
            var datatable = $('#table_ar').KTDatatable({
                data: {
                    ...
                    source: {
                        read: {
                            url: '/AccountReceiveable/GetDataGroup',
                            method: 'GET',
                ...
                columns: [
                    ...
                    {
                        field: 'nama_group',
                        title: 'Nama Grup Pelanggan',
                        width: 'auto',
                        autoHide: false,
                        textAlign: 'left',
                        template: function(row) {
                            var txt = "";
                                txt += '<span class="font-weight-bold">'+ucwords(row.nama_group);
                                txt += '</span>';
                                return txt;
                        },
                    },
                    ...
                    // },
                    {
                        ...
                        template: function(row) {
                            ...
                            var txtAction = '<a href="{{route("AccountReceiveable.DetailGroup", "idAr")}}" class="btn btn-outline-primary btn-sm mr-3"><i class="flaticon-eye"></i>Lihat</a>';
            ...
            $('#id_grup').on('change', function() {
                datatable.search($(this).val(), 'nama_group');
            });
